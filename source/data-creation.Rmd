...
cs_validate(yearlist20, year = 2020)
#correct November 2020 issues
# extract data and unit test column name
nov20 <- cs_extract_month(yearlist20, month = "November")
...
# replace data
yearlist20 <- cs_replace_month(yearlist20, month = "November", monthData = nov20)

# validate
expect_equal(cs_validate(yearlist20, year = "2020"), TRUE)
...
ytdCrimes20 <- ytdCrimes20 %>% 
  ...
  mutate(nbhd_name = ifelse(grepl("38", neighborhood), "Central West End",
...
```{r}
#monthCrimes20_nocoord <- monthCrimes20_all %>% 
#  filter(., missing == TRUE)

#monthCrimes20 <- monthCrimes20 %>% 
#  filter(., missing == FALSE)

#geocoder <- gw_build_geocoder(return = c("coords", "parcel", "zip"), crs = 4296)

#geocode current year
#monthCrimes20_nocoord %>%
#  mutate(address = paste(cad_address, cad_street, sep = " ")) -> monthCrimes20_nocoord

#remove xy columns
#nocoord_test <- dplyr::select(nocoord, -x_coord, -y_coord)

#nocoord_geo <- gw_geocode(monthCrimes20_nocoord, type = "local", var = "address",  class = "tibble", geocoder = geocoder)

#extract xy coordinates from geometry
#nocoord <- gw_get_coords(nocoord, names = c("x_coord","y_coord"), crs = 4296)
#merge back to ytdCrimes20
#ytdCrimes20 <- rbind(ytdCrimes20, nocoord)
...
#remove observations that are missing xy coordinates
#nocoord <- dplyr::filter(totalCrimes19, x_coord == 0 | y_coord == 0)
#geocode current year
#nocoord <- gw_geocode(nocoord, type = "local", class = "sf", address = "address", geocoder = geocoder)
#remove xy columns
#nocoord <- dplyr::select(nocoord, -x_coord, -ycoord)
#extract xy coordinates from geometry
#nocoord <- gw_get_coords(nocoord, names = c("x_coord","y_coord"), crs = 4296)
#merge back to ytdCrimes20
#totalCrimes19 <- rbind(totalCrimes19, nocoord)
...
# all crimes 
totalCrimes19_sf <- cs_projectXY(totalCrimes19, varX = x_coord, varY = y_coord, crs = 4296)
ytdCrimes19_sf <- cs_projectXY(ytdCrimes19, varX = x_coord, varY = y_coord, crs = 4296)
ytdCrimes20_sf <- cs_projectXY(ytdCrimes20, varX = x_coord, varY = y_coord, crs = 4296)

monthCrimes19_sf <- cs_projectXY(monthCrimes19, varX = x_coord, varY = y_coord, crs = 4296)
monthCrimes20_sf <- cs_projectXY(monthCrimes20, varX = x_coord, varY = y_coord, crs = 4296)

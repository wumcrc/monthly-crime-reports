...
report%>%
  ...
  ph_with(value = "Forest Park Southeast: Summary Notes", location = ph_location_type(type = "title"))%>%
    ph_with(value = empty_content(), location = ph_location_type(type = "body"))%>%
    ph_add_par(level = 1L)%>%
    ph_add_text(str = sprint_1, style = fp_text(color = "black", font.size = 28, bold = TRUE, underlined = TRUE))%>%
    ph_add_par(level = 2L)%>%
    ph_add_text(str = paste("compared to ", params$month, " ", params$pastyear, " (", tc19, " total crimes)", sep = ""), 
                style = fp_text(color = 'black', font.size = 28)) %>% 
    ph_add_text(str = paste(pct_chg_mth, "change ", sep = " "),
                style = fp_text(color = ifelse(pct_chg_mth1 == 0, "#00B0F0", ifelse(pct_chg_mth < 0, "#00B050", "red")), font.size = 28)) %>%
    ph_add_par(level = 2L) %>%
    ph_add_text(str = paste(" in", params$month, params$year, sep = " "), 
                style = fp_text(color = "black", font.size = 28))%>%
    ph_add_text(str = paste(cap20, "crime(s) against persons", sep = " "), 
                style = fp_text(color = "black", font.size = 28, bold = TRUE)) %>%
    ph_add_par(level = 3L) %>%
    ph_add_text(str = paste(" compared to ", params$month, " ", params$pastyear, " (", cap19, " crimes against persons)", sep = ""), style = fp_text(color = "black", font.size = 28))%>%
    ph_add_text(str = paste(pct_chg_cap, "change", sep = " "), 
                style = fp_text(color = ifelse(pct_chg_cap1 == 0, "#00B0F0", ifelse(pct_chg_cap < 0, "#00B050", "red")), font.size = 28))%>%
    ph_add_par(level = 1L) %>%
    ph_add_text(str = sprint_2, style = fp_text(color = "black", font.size = 28, bold = TRUE, underlined = TRUE))%>%
    ph_add_par(level = 2L)%>%
    ph_add_text(str = paste("compared to this time in ", params$pastyear, " (", tc_ytd19, " total crimes)", sep = ""), 
                style = fp_text(color = 'black', font.size = 28)) %>% 
  ph_add_text(str = paste(pct_chg_ytd, "change ", sep = " "), 
              style =  fp_text(color = ifelse(pct_chg_ytd1 == 0, '#00B0F0', ifelse(pct_chg_ytd < 0, '#00B050', 'red')), font.size = 28)) %>%
    ph_add_par(level = 2L) %>%
    ph_add_text(str = paste(" in", params$year, sep = " "), 
                style = fp_text(color = "black", font.size = 28))%>%
    ph_add_text(str = paste(cap20_ytd, "crime(s) against persons", sep = " "), 
                style = fp_text(color = "black", font.size = 28, bold = TRUE)) %>%
    ph_add_par(level = 3L) %>%
    ph_add_text(str = paste(" compared to this time in ", params$pastyear, " (", cap19_ytd, " crimes against persons)", sep = ""), 
                style = fp_text(color = "black", font.size = 28))%>%
    ph_add_text(str = paste(pct_chg_ytd_cap, "change", sep = " "), 
                style = fp_text(color = ifelse(pct_chg_ytd_cap1 == 0, "#00B0F0", ifelse(pct_chg_ytd_cap < 0, "#00B050", "red")), font.size = 28)) -> report
```

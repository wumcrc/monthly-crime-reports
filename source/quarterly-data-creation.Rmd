...
dir.create(here(file.path("results", params$year, params$quarter)))
dir.create(here(file.path("results", params$year, params$quarter, "ac")))
dir.create(here(file.path("results", params$year, params$quarter, "bot")))
dir.create(here(file.path("results", params$year, params$quarter, "cwe")))
dir.create(here(file.path("results", params$year, params$quarter, "dbp")))
dir.create(here(file.path("results", params$year, params$quarter, "district-2")))
dir.create(here(file.path("results", params$year, params$quarter, "district-5")))
dir.create(here(file.path("results", params$year, params$quarter, "fp")))
dir.create(here(file.path("results", params$year, params$quarter, "fpse")))
dir.create(here(file.path("results", params$year, params$quarter, "lp")))
dir.create(here(file.path("results", params$year, params$quarter, "sdb")))
dir.create(here(file.path("results", params$year, params$quarter, "vd")))
dir.create(here(file.path("results", params$year, params$quarter, "vp")))
dir.create(here(file.path("results", params$year, params$quarter, "we")))
dir.create(here(file.path("results", "presentations",  params$year)))
dir.create(here(file.path("results", "presentations",  params$year, params$quarter)))
```
...
i <- cs_create_index()
past_yearlist <- cs_get_data(year = params$pastyear, index = i)
pres_yearlist <- cs_get_data(year = params$year, index = i)        # Constructs a table for finding a table of crime data
cs_validate(past_yearlist, year = 2019)                            # Identifies problems with SLMPD Data
cs_validate(pres_yearlist, year = 2020)                            # Identifies problems with SLMPD Data
```

#### Standardize Data for Cleaning

```{r Standardize 2018 and 2019 Data}
pastyear <- cs_collapse(past_yearlist)
presyear <- cs_collapse(pres_yearlist) 
```
...
presyear <- presyear %>% 
  ...
  # Recode values to abbreviated neighborood names
  mutate(abbr = ifelse(grepl("38", neighborhood), "cwe",
...
presyear$monthVar <- month(as.Date(presyear$monthVar, 
                                      format="%d/%m/%Y"), label = TRUE)                   # Pulls out the Month based on the Date | i.e 10/01/2018 = "October" 

presyear$neighborhood <- as.numeric(presyear$neighborhood)                          # Formats the neighborhood variable as numeric
presyear$tod <- strptime(presyear$tod, tz = "America/Chicago", "%H:%M")             # Strips & Reformats the structure of the time variable
presyear$tod <- format(presyear$tod, format = "%H%M%S")
...
presqrt <- presyear %>% 
    filter(dateOcc >= as.Date(params$qrtstart) & dateOcc <= as.Date(params$qrtend))  # Filters dataset of crimes that happened in the quarter determined by the parameters
```
...
pastyear$monthVar <- month(as.Date(pastyear$monthVar, format="%d/%m/%Y"), label = TRUE)

pastyear$neighborhood <- as.numeric(pastyear$neighborhood)

pastyear$tod <- strptime(pastyear$tod, tz = "America/Chicago", "%H:%M")
pastyear$tod <- format(pastyear$tod, format = "%H%M%S")
...
pastqrt <- pastyear %>% 
    filter(dateOcc >= as.Date(params$qrtstart2) & dateOcc <= as.Date(params$qrtend2)) 

save(presqrt, pastqrt, larceny, file = here("data", "quarterly-crime-data.rda"))
```
...
# all crimes 
pastyear_sf <- cs_projectXY(pastyear, varX = x_coord, varY = y_coord, crs = 102696)
ytdCrimes19_sf <- cs_projectXY(ytdCrimes19, varX = x_coord, varY = y_coord, crs = 102696)
presyear_sf <- cs_projectXY(presyear, varX = x_coord, varY = y_coord, crs = 102696)
...
mc_crimes_ytd20_sf <- st_intersection(presyear_sf, med_campus)
mc_crimes_ytd20 <- mc_crimes_ytd20_sf %>% 
   as.data.frame()

mc_crimes_total19_sf <- st_intersection(pastyear_sf, med_campus)
...
# Save
save(pastyear_sf, ytdCrimes19_sf, presyear_sf, monthCrimes19_sf, monthCrimes20_sf, mc_crimes_month19_sf, mc_crimes_month20_sf, mc_crimes_ytd19_sf, mc_crimes_total19_sf, mc_crimes_ytd20_sf, mc_larcenies_sf, ctx_crimes_sf, dst_2_pop_sf, dst_5_pop_sf, file = here("data", "spatial-crime-data.rda"))

save(ytdCrimes19, presyear, monthCrimes19, monthCrimes20, pastyear, larceny, mc_crimes_month19, mc_crimes_month20, mc_crimes_ytd19, mc_crimes_total19, mc_crimes_ytd20, mc_larcenies, dst_2, dst_5, file = here("data", "crime-data.rda"))

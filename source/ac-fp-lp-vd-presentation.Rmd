...
report%>%
  ...
  ph_with(value = "Academy-Sherman Park, Fountain Park, Lewis Place, Vandeventer", location = ph_location_type(type = "ctrTitle"))%>%
...
name <- comment(i)
  if(name %in% monthCrimes_pres$tiles){
  nbhd_pres <- filter(monthCrimes_pres, tiles == name)
  neighborhood <- unique(nbhd_pres$nbhd_name)
abbr <- unique(nbhd_pres$abbr)
num <- unique(nbhd_pres$neighborhood)


tc_past <- nrow(monthCrimes_past[monthCrimes_past$neighborhood == num,])
tc_pres <- nrow(monthCrimes_pres[monthCrimes_pres$neighborhood == num,])
cap_past <- nrow(monthCrimes_past[monthCrimes_past$neighborhood == num & monthCrimes_past$crimeCatNum == 4 | monthCrimes_past$neighborhood == num & monthCrimes_past$crimeCatNum == 3,])
cap_pres <- nrow(monthCrimes_pres[monthCrimes_pres$neighborhood == num & monthCrimes_pres$crimeCatNum == 4 | monthCrimes_pres$neighborhood == num & monthCrimes_pres$crimeCatNum == 3,])

tc_ytd_past <- nrow(ytdCrimes_past[ytdCrimes_past$neighborhood == num,])
tc_ytdCurrent <- nrow(ytdCrimes[ytdCrimes$neighborhood == num,])
cap_past_ytd <- nrow(ytdCrimes_past[ytdCrimes_past$neighborhood == num & ytdCrimes_past$crimeCatNum == 4 | ytdCrimes_past$neighborhood == num & ytdCrimes_past$crimeCatNum == 3,])
cap_pres_ytd <- nrow(ytdCrimes[ytdCrimes$neighborhood == num & ytdCrimes$crimeCatNum == 4 | ytdCrimes$neighborhood == num & ytdCrimes$crimeCatNum == 3,])
...
common_cols <- intersect(colnames(yr_pres[, (params$num + 1)]), colnames(yr_past[, (params$num + 1)]))
...
tmap_save(total_tm, file = here("results", params$year, params$month, abbr, paste(abbr, "_total_tm.jpeg", sep = "")), width = 9, height = 5, units = "in")
...
#District 5 Map & Explanation
```{r District 5 Density Map Explanation Prep, include = FALSE}
#add District 5 Crime Rates Map to powerpoint report
report%>%
  add_slide()%>%
...
for (i in list(ac_tiles, fp_tiles, lp_tiles, vd_tiles)) {
  name <- comment(i)
  pres_sf <- filter(monthCrimes_pres_sf, tiles == name)
  pres_df <- filter(monthCrimes_pres, tiles == name)
  neighborhood <- unique(pres_sf$nbhd_name)
abbr <- unique(pres_sf$abbr)
num <- unique(pres_sf$neighborhood)
...
ggsave(here("results", params$year, params$month, abbr, paste(abbr, "_crime_weekday_graph.jpeg", sep = "")), dpi = 500)
...
print(report, target = here::here("test.pptx"))
```
...
file <- read_pptx(here::here("results", "presentations",  params$year, params$month, paste("ac-fp-lp-vd-", params$month, params$year, ".pptx", sep = "")))
...
file.remove(file.path(shinydr, dir(path = shinydr, pattern = "Vandeventer_crime")))
...
file.remove(file.path(mydir, dir(path = mydir, pattern = paste("ac-fp-lp-vd-", params$month, params$year, ".pdf", sep=""))))

#convert & save powerpoint as PDF to presentations subfolder
convert_to_pdf(path = here::here("results", "presentations",  params$year, params$month, paste("ac-fp-lp-vd-", params$month, params$year, ".pptx", sep = "")),
               pdf_file = sub("[.]pptx", ".pdf", paste(mydir, "ac-fp-lp-vd-", params$month, params$year, ".pptx", sep = "")))
```

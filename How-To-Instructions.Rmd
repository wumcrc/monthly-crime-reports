...
#Step 7
After you have updated the params, the next step is to update line 211 of the code. This line of code summarises total crimes per month but needs to be manually changed every time you run the code. All you need to do is append the current month of evaluation to the line of code. say, we are running the code for July (which is in the Jul-Aug period). Line 211 (before we change it) will look as follows:
...
After the last month that is there. The final code for line 211 for the month of July will look as follows:

              dplyr::summarise(Jan = sum(Jan), Feb = sum(Feb), Mar = sum(Mar), Apr = sum(Apr), May = sum(May), Jun = sum(Jun), Jul = sum(Jul))%>%

When we want to run the next month, August, we want to make sure that all preceding months (including July) are also in line 211. So, once we run the code for July and want to run the code for August we just add August to the end of the line of code as follows:

              dplyr::summarise(Jan = sum(Jan), Feb = sum(Feb), Mar = sum(Mar), Apr = sum(Apr), May = sum(May), Jun = sum(Jun), Jul = sum(Jul), Aug = sum(Aug))%>%

#Step 8
Once you have updated line 211 of `fpse-bot-cwe-mc-presentation.Rmd` you can run the code!

#Step 9
Now that you have run the `fpse-bot-cwe-mc-presentation.Rmd` you can repeat steps 3-5 for the `sdb-dbp-we-vp-presentation.Rmd` and `ac-fp-lp-vd-presentation.Rmd`. The only difference is that the line of code to manually change will be 212 instead of 211! It is not important which one you run first, just be sure to change the params and line 212! They are the exact same as the params for the `fpse-bot-cwe-mc-presenation.Rmd`. 

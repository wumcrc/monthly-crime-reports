...
---
#IMPORTANT
Make sure to update line 214 before running this .Rmd. Refer to the `How-To-Instructions.Rmd` for more information.
...
overview <- temp_df%>%
  dplyr::group_by(type)%>%
  dplyr::summarise(Jan = sum(Jan), Feb = sum(Feb), Mar = sum(Mar), Apr = sum(Apr), May = sum(May), Jun = sum(Jun), Jul = sum(Jul), Aug = sum(Aug), Sep = sum(Sep), Oct = sum(Oct))%>%
...
print(report, target = here::here("NIBRS", "results", "presentations", params$year, params$month, paste("ac-fp-lp-vd-", params$month, params$year, ".pptx", sep = "")))
...
convert_to_pdf(path = here::here("NIBRS", "results", "presentations", params$year, params$month, paste("ac-fp-lp-vd-", params$month, params$year, ".pptx", sep = "")),

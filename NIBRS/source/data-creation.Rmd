...
#IMPORTANT
This notebook needs to be manually updated both in the paramaters and line 123. DO NOT run this code unless you have updated both of these things!
...
tot <- base%>%
  group_by(Crime, type)%>%
  summarise(Jan = sum(Jan), Feb = sum(Jan), Mar = sum(Mar), Apr = sum(Apr))
#save output
save(base, tot, file = here::here("NIBRS", "data", params$year, paste(params$month1, "-", params$month2, sep = ""), "clean-crime-data.rda"))
```

```{r, eval = FALSE}
#Density Map Iteration Code
